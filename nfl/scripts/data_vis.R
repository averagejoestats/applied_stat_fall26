...
for(j in 1:nrow(team_stats)){
    ...
    i2025 <- dat$year == 2025
    x <- dat$outcome
    team_stats$wins_2024[j] <- sum( x[ ihome & i2024 ]) + sum(1 - x[ iaway & i2024 ] )
    team_stats$wins_2025[j] <- sum( x[ ihome & i2025 ]) + sum(1 - x[ iaway & i2025 ] )
...
team_stats$abbr <- c("KC","PHL","ATL","BUF","NO","CHI","CIN","IND","MIA","NYG","SEA","LAC",
                     "CLE","TAM","DET","SF","CAR","JAX","GB","DAL","MN","NE","WAS","TN",
                     "BAL","AZ","DEN","HOU","NYJ","PIT","LV","LAR")

    
# first try
plot(team_stats$wins_2024, team_stats$wins_2025, xlim = c(0,17), ylim = c(0,17),
...
text( team_stats$wins_2024, team_stats$wins_2025 + team_stats$offset, team_stats$abbr, col = colors, font = 2)
axis(1, at = 0:17)
axis(2, at = 0:17)
box()
